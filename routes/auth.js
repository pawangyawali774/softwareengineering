...
module.exports = (db) => {
    ...
    router.post('/auth/signup', async (req, res) => {
        ...
        db.query('INSERT INTO users (name, email, location, password, points) VALUES (?, ?, ?, ?, 10)',
    ...
    router.post('/auth/login', (req, res) => {
        ...
        db.query('SELECT * FROM users WHERE email = ? LIMIT 1', [email], async (err, results) => {
            ...
            // Award 1 point per login
            db.query('UPDATE users SET points = points + 1 WHERE id = ?', [results[0].id], () => {
                results[0].points = (results[0].points || 0) + 1;
